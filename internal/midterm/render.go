...
// This context should only use in callback
// or after Lock() before unlock()
type LockedTerminalContext interface {
	PlainRender() []string
	PlainRenderLine(row int) string
}

type lockedTerminalContextImpl struct {
	vt *Terminal
}

var _ LockedTerminalContext = (*lockedTerminalContextImpl)(nil)

func (ltc *lockedTerminalContextImpl) PlainRender() []string {
	return ltc.vt.unsafePlainRender()
}

func (ltc *lockedTerminalContextImpl) PlainRenderLine(row int) string {
	return ltc.vt.unsafePlainRenderLine(row)
}
...
func (vt *Terminal) PlainRender() []string {
	vt.mut.Lock()
	defer vt.mut.Unlock()
	return vt.unsafePlainRender()
}

func (vt *Terminal) unsafePlainRender() (out []string) {
	out = make([]string, vt.Height)
	for i := range vt.Height {
		out[i] = string(vt.Content[i])
	}
	return
}

func (vt *Terminal) PlainRenderLine(row int) string {
	vt.mut.Lock()
	defer vt.mut.Unlock()
	return vt.unsafePlainRenderLine(row)
}

func (vt *Terminal) unsafePlainRenderLine(row int) string {
